...
export const isDevelopment: (() => boolean) = once(() => {
  try {
...
export const isRunningInTest: (() => boolean) = once(() => {
  if (isRunningInClient()) {
    return global.atom.inSpecMode();
  } else {
...
export function isRunningInClient(): boolean {
  return typeof global.atom !== 'undefined';
}
...
export function getAtomNuclideDir(): string {
  ...
  const nuclidePackageModule = global.atom.packages.getLoadedPackage('nuclide');
...
export function getRuntimePath(): string {
  ...
  //          "C:\Atom\resources"
  // $FlowExpectedError[prop-missing] process.resourcePath missing
