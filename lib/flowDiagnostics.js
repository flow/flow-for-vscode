	...
	if (rawDiag && rawDiag.messages) {
		rawDiag.messages.forEach((message) => {
			...
			const
				...
				file = baseMessage.range.file
				
			let diag = {
				severity: level,
				startLine: baseMessage.range.start.line,
				startCol: baseMessage.range.start.column,
				endLine: baseMessage.range.end.line,
				endCol: baseMessage.range.end.column
			}
