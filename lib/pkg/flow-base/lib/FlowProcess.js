...
export class FlowProcess {
  ...
  dispose(): void {
    ...
    if (this._startedServer && getStopFlowOnExit()) {
      ...
      return this._startedServer.then(serverProcess => serverProcess.kill('SIGKILL'));
    }
    return Promise.resolve();
  }
  ...
  async _startFlowServer(): Promise<void> {
    if (this._startedServer) {
      await this._startedServer.then(serverProcess => {
        logger.info(`The flow server (${pid}) is running.`);
      });
      return;
    }
    const pathToFlow = await getPathToFlow(this._root);
