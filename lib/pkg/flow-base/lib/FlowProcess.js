...
export class FlowProcess {
  ...
  constructor(root: string) {
    ...
    this._serverStatus.subscribe(status => {
      this._flowServerStatus = status;
      logger.info(`[${status}]: Flow server in ${this._root}`);
    });
    const startFlowServer = () => {
      if (this._flowServerStatus === ServerStatus.NOT_RUNNING) {
        return this._startFlowServer();
      }
      return Promise.resolve();
    };
    const pingServer = () => this._pingServer();

    this._serverStatus.filter(x => x === ServerStatus.NOT_RUNNING).subscribe(() => {
      if (this._flowServerPromise) {
        this._flowServerPromise = this._flowServerPromise
          .then(startFlowServer, startFlowServer)
          .then(pingServer, pingServer);
      } else {
        this._flowServerPromise = startFlowServer().then(pingServer, pingServer);
      }
    });
    ...
    this._serverStatus.filter(isBusyOrInit).subscribe(() => {
      if (this._flowServerPromise)  {
        ...
        this._flowServerPromise = pingServer();
      }
    });
  ...
  dispose(): void {
    ...
    if (this._startedServer && getStopFlowOnExit()) {
      ...
      return this._startedServer.then(serverProcess => {
        logger.info(`Stop the flow server (${serverProcess.pid}).`);
  ...
  async _startFlowServer(): Promise<void> {
    if (this._startedServer) {
      await this._startedServer.then(serverProcess => logger.info(`The flow server (${serverProcess.pid}) is running.`));
      return;
    }
    const pathToFlow = await getPathToFlow(this._root);
    const args = [
    ...
    const moreArgs = this._getFlowExecOptions();
    logger.info(`Launching flow server: ${pathToFlow} ${args.join(' ')}`);
    /*
      ...
      ChildProcess object.
     */
    this._startedServer = safeSpawn(pathToFlow, args, moreArgs).then(serverProcess => {
      let isLog = false;
      const logIt = data => {
        if (!isLog) {
          isLog = true;
          logger.info(`Launched the flow server (${serverProcess.pid}).`);
        }
        const d = String.fromCharCode(...data).replace(/\n/g, '\n\t');
        logger.trace(`flow server (${serverProcess.pid}): \n\t${d}`);
      };
      serverProcess.stdout.on('data', logIt);
      serverProcess.stderr.on('data', logIt);
      serverProcess.on('exit', (code, signal) => {
        logger.info(`Terminates the flow server (${serverProcess.pid}).`)
        this._startedServer = null;
        /*
    ...
    await this._startedServer;
  }

  /** Execute Flow with the given arguments */
  _rawExecFlow(args: Array<any>, options?: Object = {}): Promise<?process$asyncExecuteRet> {
    const __rawExecFlow = async () => {
      args = [
        ...args,
        '--retry-if-init=false',
        '--retries=0',
      ...
      const label = `_rawExecFlow-${new Date().getTime()}: ${args.join(' ')}`;
      try {
        console.log(label);
        console.time(label);
        ...
        options = {...this._getFlowExecOptions(), ...options};
        try {
    ...
    if (this._rawExecFlowPromise == null) {
      this._rawExecFlowPromise = __rawExecFlow();
    } else {
      this._rawExecFlowPromise = this._rawExecFlowPromise.then(() => __rawExecFlow(), () => __rawExecFlow());
    }
    return this._rawExecFlowPromise;
  }
  ...
  _pingServer(tries?: number = 500): Promise<void> {
    const __pingServer = async () => {
      ...
      for (let i = 0; !stateChanged && i < tries; i++) {
        const label = `_pingServer-${new Date().getTime()}`;
        console.log(label);
        console.time(label);
    ...
    if (this._pingServerPromise == null) {
      this._pingServerPromise = __pingServer();
    } else {
      this._pingServerPromise = this._pingServerPromise.then(() => __pingServer(), () => __pingServer());
    }
    return this._pingServerPromise;
  }
  ...
  */
   ...
   * execFlow.
   */
  static execFlowClient(
    args: Array<any>,
    options: ?Object = {},
    root: ?string
  ): Promise<?process$asyncExecuteRet> {
    let _execFlowClient = async () => {
      args = [
        ...args,
        '--from=nuclide'
      ];
    ...
    if (execFlowClientPromise == null) {
      execFlowClientPromise = _execFlowClient();
    } else {
      execFlowClientPromise = execFlowClientPromise.then(() => _execFlowClient(), () => _execFlowClient());
    }
    return execFlowClientPromise;
  }
}
