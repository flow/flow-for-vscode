...
async function isFlowInstalled(root: ?string): Promise<boolean> {
  const flowPath = await getPathToFlow(root);
...
function filterJs({languageId}): boolean{
  return languageId === 'javascript';
}

function filterCss({languageId}): boolean{
  return languageId === 'css';
}

function findActFolder(activeDoc): Function{
  const docPath = activeDoc.uri.path;
  return function({uri: {path}}): boolean{
    return docPath.indexOf(`${path}/`) === 0;
  };
}
// In case of workspace folders, calculate the root path referencing the active editor.
function calcWorkspaceRoot(root: ?string): string{
  let workspaceRoot = root;
  if (workspaceRoot) {
    return workspaceRoot;
  }
  logger.info('The parameter "root" is lack for the flow-path calculation.');
  const {window: {activeTextEditor}, workspace: {rootPath, textDocuments, workspaceFolders}} = global.vscode;
  let activeDoc, cssDocs, jsDocs, workspaceFolder;
  if (activeTextEditor) { // If any of editors is active,
    activeDoc = activeTextEditor.document; // target its current workspace folder.
  } else if ((jsDocs = textDocuments.filter(filterJs)).length > 0) {
    // otherwise, target the workspace folder accommodating the latest-opened Javascript file.
    activeDoc = jsDocs[jsDocs.length - 1];
  } else if ((cssDocs = textDocuments.filter(filterCss)).length > 0) {
    // otherwise, target the workspace folder accommodating the latest-opened CSS file.
    activeDoc = cssDocs[cssDocs.length - 1];
  } else if (textDocuments.length > 0) {
    // otherwise, target the workspace folder accommodating the latest-opened file.
    activeDoc = textDocuments[textDocuments.length - 1];
  }
  if (activeDoc && (workspaceFolder = workspaceFolders.find(findActFolder(activeDoc)))) {
...
function getPathToFlow(root: ?string): Promise<string> {
  const workspaceRoot = calcWorkspaceRoot(root);
  if (!global.cachedPathToFlowBin[workspaceRoot]) {
    ...
    global.cachedPathToFlowBin[workspaceRoot] = (async () => {
      let rs;
      if (shouldUseNodeModule && await canFindFlow(nodeModuleFlowPath)){
        rs = nodeModuleFlowPath;
        ...
        rs = nodeModuleFlowLocation(extensionRoot)
      }
      logger.info("Path to Flow: " + rs);
      return rs;
    })();
  }
  return global.cachedPathToFlowBin[workspaceRoot];
}
