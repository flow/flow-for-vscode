...
async function getShasums(flowBinModulePath: string, logger: Logger): Promise<Buffer> {
    ...
    try {
        ...
        return await readFile(path.join(extensionPath, 'PAST_FLOW_BIN_SHASUMS.txt'));
    }
}
...
function getShasum(shasums: string, flowBinDirName: string, flowBinName: string): string {
    ...
    if (!shasum_line) {
        throw new Error(`Failed to find SHASUM256.txt line containing ${flowBinRelativePath}`);
    }
